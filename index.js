...
client.on('ready', () => {
    ...
    client.user.setPresence({ activity: { name: 'LPMC reklamowy 1.0 ┃ pomoc' }, status: 'online' })
    let numer = 1
      setInterval(() => {
...
client.on('message', async(message)=>{
    if (message.content === "<@!idbota>") {
        ...
        .setFooter("lpmc reklamowy - Twój innowacyjny bot reklamowy!")
...
client.on("guildCreate", async guild => {
    ...
    const logChannel = await client.channels.cache.get('903013420322942976');
...
client.on("guildDelete", async guild => {
    ...
    const logChannel = await client.channels.cache.get(idkanalulogi');
